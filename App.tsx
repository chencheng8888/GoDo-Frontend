...
const App: React.FC = () => {
			...
			<div className="min-h-screen flex items-center justify-center bg-slate-100 p-4">
				<div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8">
					<div className="text-center mb-8">
						<div className="mx-auto w-12 h-12 bg-primary-600 rounded-xl flex items-center justify-center text-white text-2xl font-bold mb-4 shadow-lg shadow-primary-600/30">G</div>
					...
					<form onSubmit={handleLogin} className="space-y-6">
						...
						{loginError && <div className="text-sm text-red-600 bg-red-50 p-3 rounded-lg text-center">{loginError}</div>}
					...
					<div className="mt-6 text-center text-xs text-slate-400">GoDo Task Scheduling System &copy; 2024</div>
				</div>
			</div>
		);
	}
	...
	// Dashboard Layout
	return (
		<AppLayout currentView={currentView} username={session.username} onViewChange={setCurrentView} onLogout={handleLogout}>
			{currentView === 'tasks' ? <TaskView username={session.username} /> : currentView === 'files' ? <FileView /> : <LogView username={session.username} />}
		</AppLayout>
	);
};
