...
export const LogView: React.FC<LogViewProps> = ({ username }) => {
  ...
  const renderContentDetail = (contentStr: string) => {
    try {
        const parsed = JSON.parse(contentStr);
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl z-10 flex flex-col max-h-[90vh] animate-fade-in-up">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-2xl">
                <div>
                    ...
                    <span className="text-xs font-mono text-slate-500">ID: {selectedLog.ID} • TaskID: {selectedLog.TaskId}</span>
                </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
                <div>
                    ...
                    <pre className="bg-slate-900 text-slate-200 p-3 rounded-lg font-mono text-xs overflow-x-auto whitespace-pre-wrap">
                        {renderContentDetail(selectedLog.Content)}
                    </pre>
                </div>
                ...
                {selectedLog.ErrOutput && selectedLog.ErrOutput.length > 0 && (
                    <div>
                        ...
                        <pre className="bg-red-50 text-red-800 p-4 rounded-lg font-mono text-xs overflow-x-auto whitespace-pre-wrap border border-red-100">
                            {selectedLog.ErrOutput}
                        </pre>
                    </div>
                )}
                ...
                <div>
                    ...
                    <pre className="bg-slate-50 text-slate-700 p-4 rounded-lg font-mono text-xs overflow-x-auto whitespace-pre-wrap border border-slate-200 min-h-[100px]">
                        {selectedLog.Output || <span className="text-slate-400 italic">No output</span>}
                    </pre>
                </div>
            </div>
